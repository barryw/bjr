...
#
class StatsPerHourJob < ApplicationJob
  queue_as :stats_runner

  def perform
    User.find_each do |user|
      logger.debug "Processing hourly job stats for user #{user.username}"
      ...
      while current_dt < end_dt
        runs = JobRun.runs_in_range(user.id, current_dt - 1.hour, current_dt).count
        failed = JobRun.fails_in_range(user.id, current_dt - 1.hour, current_dt).count

        avg_runtime = JobRun.avg_runtime_in_range(user.id, current_dt - 1.hour, current_dt)
        max_runtime = JobRun.max_runtime_in_range(user.id, current_dt - 1.hour, current_dt)
        min_runtime = JobRun.min_runtime_in_range(user.id, current_dt - 1.hour, current_dt)

        JobStat.create(user_id: user.id, runs: runs, failed: failed, avg_runtime: avg_runtime,
                       min_runtime: min_runtime, max_runtime: max_runtime, start_dt: current_dt - 1.hour,
                       end_dt: current_dt, period: :hour)

        current_dt = current_dt + 1.hour
      end
