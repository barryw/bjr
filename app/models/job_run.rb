...
class JobRun < ApplicationRecord
  ...
  scope :job_schedule_diffs, ->(job_id) { joins(:job).where(jobs: { id: job_id }, is_manual: false).where.not(end_time: nil).order(end_time: :asc).pluck(:schedule_diff_in_seconds) }
  scope :avg_job_lag_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }, is_manual: false).where.not(end_time: nil).average(:schedule_diff_in_seconds) }
  scope :max_job_lag_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }, is_manual: false).where.not(end_time: nil).maximum(:schedule_diff_in_seconds) }
  scope :min_job_lag_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }, is_manual: false).where.not(end_time: nil).minimum(:schedule_diff_in_seconds) }

  scope :job_runtimes, ->(job_id) { joins(:job).where(jobs: { id: job_id }).where.not(end_time: nil).order(end_time: :asc).pluck(Arel.sql('timestampdiff(microsecond, start_time, end_time)/1000000')) }
  scope :avg_runtime_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }).where.not(end_time: nil).average(Arel.sql('timestampdiff(microsecond, start_time, end_time)/1000000')) }
  scope :max_runtime_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }).where.not(end_time: nil).maximum(Arel.sql('timestampdiff(microsecond, start_time, end_time)/1000000')) }
  scope :min_runtime_for_job, ->(job_id) { joins(:job).where(jobs: { id: job_id }).where.not(end_time: nil).minimum(Arel.sql('timestampdiff(microsecond, start_time, end_time)/1000000')) }
