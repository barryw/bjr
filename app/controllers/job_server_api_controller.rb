...
class JobServerApiController < ApplicationController
  ...
  def minutely_job_stats
    stats = JobStat.mine(current_user).where(period: :minute, start_dt: @start_date..@end_date).order(created_at: :asc)
  ...
  def hourly_job_stats
    stats = JobStat.mine(current_user).where(period: :hour, start_dt: @start_date..@end_date).order(created_at: :asc)
  ...
  #
  def daily_job_stats
    stats = JobStat.mine(current_user).where(period: :day, start_dt: @start_date..@end_date).order(created_at: :asc)
  ...
  def weekly_job_stats
    stats = JobStat.mine(current_user).where(period: :week, start_dt: @start_date..@end_date).order(created_at: :asc)
  ...
  def date_range
    @start_date = date_parse(params[:start_date]) || DateTime.now - 1.hour
    @end_date = date_parse(params[:end_date]) || DateTime.now
  end
